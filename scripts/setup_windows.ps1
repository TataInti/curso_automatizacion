...
[CmdletBinding()]
param(
    [string]$RepoUrl = "https://github.com/TataInti/curso_automatizacion.git",
    [string]$InstallRoot = (Join-Path $env:USERPROFILE "GitHub"),
    [string]$ProjectName = "curso_automatizacion",
...
function Install-Python312 {
    ...
    $PythonPatchVersion = "3.12.10"
    $fileName = "python-$PythonPatchVersion$suffix.exe"
    $installerPath = Join-Path $env:TEMP $fileName
    $installerUrl = "https://www.python.org/ftp/python/$PythonPatchVersion/$fileName"
...
function Sync-Repository {
    ...
    if (Test-Path -LiteralPath (Join-Path $repoPath ".git")) {
        ...
        if ($LASTEXITCODE -ne 0 -or $origin.Trim() -ne $RepoUrl) {
            throw "La carpeta $repoPath apunta a otro repositorio. Elegi otra ubicacion con -InstallRoot o -ProjectName."
        }
...
function Initialize-VirtualEnvironment {
    ...
    if (-not (Test-Path -LiteralPath $requirementsPath)) {
        throw "No se encontro requirements.txt en $RepoPath. Actualiza el repositorio y vuelve a intentar."
    }
    $createVenv = $true

    if (Test-Path -LiteralPath $venvPython) {
        $venvVersion = & $venvPython -c "import sys; print('.'.join(map(str, sys.version_info[:3])))" 2>$null
        if ($LASTEXITCODE -eq 0 -and $venvVersion.Trim().StartsWith("$PythonVersion.")) {
            Write-Host "OK: el entorno virtual ya existe en $venvPath."
            $createVenv = $false
        }
        else {
            throw "El entorno virtual existente no usa Python $PythonVersion. Revisa .venv manualmente antes de continuar."
        }
    }
    ...
    Invoke-External $venvPython @("-m", "pip", "install", "--upgrade", "pip", "setuptools", "wheel") "Actualizando herramientas base del entorno"
    ...
    Invoke-External $venvPython @("-m", "ipykernel", "install", "--user", "--name", "curso-automatizacion-python312", "--display-name", "Python 3.12 (.venv curso_automatizacion)") "Registrando kernel de Jupyter"
}
...
Write-Host "Kernel: Python 3.12 (.venv curso_automatizacion)"
